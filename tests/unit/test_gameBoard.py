...
class TestGameBoard:
    ...
    def test_create_gameBoard(self):
        assert gameBoard.board == self.cards
    ...
    def test_gameBoard_move(self):
        ...
        gameBoard.move(3, Move.UP)
        assert gameBoard.board == self.cards

        gameBoard.move(0, Move.RIGHT)
        # We should disable black autoformating here, since that put the card in this way is way easier to understand actual gameboard
        # fmt: off
        assert gameBoard.board == [
        ...
        with pytest.raises(InvalidDirectionError):
            gameBoard.move(0, Move.UP)
            gameBoard.move(0, Move.LEFT)

            gameBoard.move(1, Move.UP)

            gameBoard.move(2, Move.UP)
            gameBoard.move(2, Move.RIGHT)

            gameBoard.move(3, Move.LEFT)

            gameBoard.move(5, Move.RIGHT)

            gameBoard.move(6, Move.LEFT)
            gameBoard.move(6, Move.DOWN)

            gameBoard.move(7, Move.DOWN)

            gameBoard.move(8, Move.RIGHT)
            gameBoard.move(8, Move.DOWN)
